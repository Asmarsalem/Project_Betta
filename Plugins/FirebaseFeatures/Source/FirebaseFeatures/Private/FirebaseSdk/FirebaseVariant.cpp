...
#define FIREBASE_FEATURES_HAS_STRINGBUILDER \
	(ENGINE_MAJOR_VERSION >= 5 || (ENGINE_MAJOR_VERSION == 4 && ENGINE_MINOR_VERSION > 24))
...
FString FFirebaseVariant::AsString() const
{
	if (!Variant.is_string())
	{
		...
		if (Variant.is_bool())
		{
			return Variant.bool_value() ? TEXT("True") : TEXT("False");
		}

		if (Variant.is_double())
		{
			return FString::Printf(TEXT("%lf"), Variant.double_value());
		}

		if (Variant.is_int64())
		{
			return FString::Printf(TEXT("%lld"), Variant.int64_value());
		}
		...
		{
			return FString::Printf(TEXT("[VECTOR SIZE %d]"), Variant.vector().size());
		}

		if (Variant.is_map())
		{
			return FString::Printf(TEXT("[MAP SIZE %d]"), Variant.map().size());
		}

		if (Variant.is_blob())
		{
			return FString::Printf(TEXT("[BLOB SIZE %d]"), Variant.blob_size());
		}
...
FString UVariantLibrary::ToJson(const FFirebaseVariant& Value)
{
	return Value.ToJson();
}
...
static FString ToJson(const firebase::Variant& Variant)
{
	constexpr size_t BufferSize = 1024;

#if FIREBASE_FEATURES_HAS_STRINGBUILDER
	using FStringBuilder = TStringBuilder<BufferSize>;
#else
	using FStringBuilder = FString;
#endif

	switch (Variant.type())
	{
	case firebase::Variant::kTypeBool: return Variant.bool_value() ? TEXT("true") : TEXT("false");
	case firebase::Variant::kTypeDouble: return FString::SanitizeFloat(Variant.double_value());
	case firebase::Variant::kTypeInt64: return FString::Printf(TEXT("%lld"), Variant.int64_value());
	case firebase::Variant::kTypeNull: return TEXT("null");
	case firebase::Variant::kTypeStaticString: return FString::Printf(TEXT("\"%s\""), UTF8_TO_TCHAR(Variant.string_value()));
	case firebase::Variant::kTypeMap:
	{
		...
		for (const auto& Field : Map)
		{
			...
			Builder += ToJson(Field.first);
			Builder += TEXT(":");
			Builder += ToJson(Field.second);
		}
	...
	case firebase::Variant::kTypeVector: 
	{
		...
		Builder += ToJson(Vector[0]);

		for (size_t i = 1; i < Vector.size(); ++i)
		{
			Builder += TEXT(",");
			Builder += ToJson(Vector[i]);
		}
	...
	return TEXT("{\"error\":\"unknown_variant\"}");
}

FString FFirebaseVariant::ToJson() const
{
