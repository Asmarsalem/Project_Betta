...
FCredential FAppleSignInCredential::ToCredential() const
{
#if WITH_FIREBASE_AUTH
	return firebase::auth::OAuthProvider::GetCredential("apple.com", TCHAR_TO_UTF8(*IdentityToken), TCHAR_TO_UTF8(*Nonce), nullptr);
#else
	return {};
#endif
}
...
{
	FGetNativeAppleSignInCredentialCallback _Callback;
	FString _Nonce;
}
...
- (id) initWithNonce: (FString&&) Nonce callback: (FGetNativeAppleSignInCredentialCallback&&) Callback
{
	self = [super init];
    ...
    didCompleteWithAuthorization: (ASAuthorization*)authorization
{
	if ([authorization.credential isKindOfClass: [ASAuthorizationAppleIDCredential class]])
    {
		...
		FAppleSignInCredential FirCred;

		NSString* IdTokenRaw = [[NSString alloc]   initWithData: AppleIDCredential.identityToken
												       encoding: NSUTF8StringEncoding];
		NSString* AuthTokenRaw = [[NSString alloc] initWithData: AppleIDCredential.authorizationCode
												       encoding: NSUTF8StringEncoding];

		FirCred.IdentityToken     = IdTokenRaw;
		FirCred.AuthorizationCode = AuthTokenRaw;
		FirCred.State			  = AppleIDCredential.state;
		FirCred.User			  = AppleIDCredential.user;
		FirCred.FullName	      = [NSPersonNameComponentsFormatter localizedStringFromPersonNameComponents: AppleIDCredential.fullName style: NSPersonNameComponentsFormatterStyleDefault options: NSPersonNameComponentsFormatterPhonetic];
		FirCred.Email			  = AppleIDCredential.email;
		FirCred.Nonce			  = _Nonce;

		FirCred.AuthorizedScopes.Reserve(AppleIDCredential.authorizedScopes.count);
		for (NSString* scope in AppleIDCredential.authorizedScopes)
		{
			FirCred.AuthorizedScopes.Add(scope);
		}

		[IdTokenRaw   release];
		[AuthTokenRaw release];
...
void UCredentialLibrary::GetCredentialFromAppleSignIn(FString Nonce, FGetNativeAppleSignInCredentialCallback Callback)
{
#if PLATFORM_IOS && WITH_FIREBASE_AUTH
...
{
	GetCredentialFromAppleSignIn(MoveTemp(Nonce), FGetNativeAppleSignInCredentialCallback::CreateLambda(
		[Callback = MoveTemp(Callback)](bool bSuccess, FAppleSignInCredential Credential, int32 Error, FString ErrorMessage) -> void
	{
		Callback.ExecuteIfBound(bSuccess, Credential, Error, ErrorMessage);
	}));
}
